...
(0, _webpack.series)((0, _webpack.getCompileTask)(__dirname), err => {
  ...
  _commander2['default'].version(_package2['default'].version).option('-s, --server [server]', 'Execute development webpack server (all|journeyName|brandName) Ex:journeyName~brandName', 'all').option('-b, --build [build]', 'Execute specific (development|production) webpack build task (default: development)', /^(development|production)$/i, 'development').option('-l, --lint [lint]', 'Execute specific (all|configs|base|test|style) lint task(s) (default: all)', /^(all|configs|base|test|style)$/i, 'all').option('-t, --test [test]', 'Execute specific (unit|watch|watchAll) unit & coverage test task(s)  (default: unit)', /^(unit|watch|watchAll)$/i, 'unit').option('-c, --scout [scout]', 'Execute specific Journey and Brand release bundle scout Ex:journeyName~brandName').option('-m, --mockServer [mockServer]', 'Execute development JSON mock server(server|watch) (default: server)', /^(server|watch)$/i, 'server').option('-r, --checkStyle [checkStyle]', 'Execute to generate LINT checkStyle(base|style) (default: base)', /^(base|style)$/i, 'base').option('-d, --doc', 'Execute technicial document generator').parse(process.argv);
